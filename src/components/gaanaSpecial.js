...
export class GaanaSpecial extends Component {
  constructor(props) {
      super(props);
  ...
  getGaanaSpData(){
    axios('http://api.gaana.com/home/gaana-special?limit=0,10&userlanguage=Hindi',{
        method: 'GET',
       headers: {
  ...
  render() {
    console.log('Gaaana data', this.state.data);
    const dataEmpty = [0,1,2,3];
    const GDataEmprty = dataEmpty.map(function(value, key) {
    ...
    const GData =   Object.keys(data).map(key =>
    <Link to={'/Songs'} seo={data[key].seokey}>
    ...
    return (
      <Router>
          <Carousel showArrows={false} showIndicators={false} showStatus={false} showThumbs={false} centerMode centerSlidePercentage={40} swipeScrollTolerance={5}>
              {this.state.data ? GData : GDataEmprty}
              <Switch>
