...
export class Showcase extends Component {
  ...
  getShowCaseData(){
    axios('http://api.gaana.com/home/showcase?limit=0,10&userlanguage=Hindi,English',{
        method: 'GET',
       headers: {
  ...
  render() {
    ...
    const showDataEmprty = dataEmpty.map(function(value, key) {
    ...
    const showData =   Object.keys(data).map(key =>
    ...
    return (
      <Router>
          <Carousel showArrows={false} showIndicators={false} showStatus={false} showThumbs={false} centerMode centerSlidePercentage={70} swipeScrollTolerance={5}>
              {this.state.data ? showData : showDataEmprty}
              <Switch>
